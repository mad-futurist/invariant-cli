...
EXPERIMENT = REPOSITORY_ROOT / "experiments" / "sqlite_observer_demo"
...
def test_sqlite_observer_demo_end_to_end(tmp_path: Path, monkeypatch: MonkeyPatch) -> None:
    demo = tmp_path / "sqlite_observer_demo"
    shutil.copytree(EXPERIMENT, demo)
    monkeypatch.chdir(demo)

    assert runner.invoke(app, ["init", "--name", "sqlite-observer-demo"]).exit_code == 0
    ...
    assert correspondence["target"] == {
        "kind": "sqlite_field",
        "namespace": "target/modern.db",
        "identifier": "accounts[id=1].available_eur",
    }
    ...
    _reset(demo, 30000)
    held_out_source = _capture(demo, "source/*.db", "source/app.py", "7500")
    held_out_target = _capture(demo, "target/*.db", "target/app.py", "75")
